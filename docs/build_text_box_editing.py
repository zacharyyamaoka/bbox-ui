...
add a TextBox to a Block's Header's Left slot, two-click-to-edit, commit,
...
STEP_LABEL = {
    1: "1 · add a TextBox to Header's Left",
...
# navigator variant that still EXISTS today, each proving the SAME
# single-click gesture — a navigator click while a TextBox elsewhere is
# mid-edit both commits the value and selects the clicked row, in one press.
#
# WHY only one row now, where this used to sweep five: main's 2026-09-11
# "Apply the picks" commit (a53757b) independently deleted four of the five
# navigators this fix was originally verified against — shadcn, dnd-kit,
# React Aria Tree, headless-tree — and fixed react-arborist as the one that
# ships (apps/docs/src/components/create/navigator/index.ts's own doc
# comment). The historical paragraph below is left as the record of what
# was actually found and fixed at the time (the fix itself, `packages/
# panel/src/twoClickEdit.ts`'s `shouldSuppressNativeFocusShift`, is generic
# to any navigator via the shared `[data-slot="nav-row"]` marker — nothing
# about it was react-arborist-specific); only the table beneath it reflects
# what can still be proven live, on the one navigator that remains.
NAV_IDS = ["arborist"]
NAV_LABEL = {"arborist": "react-arborist"}
nav_rows_html = []
...
<h2>Run it</h2>
<p>This capture was taken against the <code>merge-text-box-editing</code> worktree (main with <code>claude/text-box-editing</code> merged in, conflicts resolved, not yet committed), on the dev server already running for it at port 4188 — never the peer worktrees' own ports (4100, 4321-4323, or the original feature branch's own 4177).</p>
<div class="run">node /home/bam/bbox-ui/.claude/worktrees/merge-text-box-editing/demos/capture-text-box-editing.mjs http://localhost:4188/create /home/bam/bbox-ui/.claude/worktrees/merge-text-box-editing/reports/media/text-box-editing</div>
<p class="meas">Built by <code>docs/build_text_box_editing.py</code>. This report and its captures live in the <strong>worktree's own</strong> <code>reports/media/</code> (not the main checkout's), per this task's explicit "work only inside this worktree" instruction. The original feature branch's own worktree (<code>text-box-editing</code>, port 4177) still exists but is no longer what this report was captured against.</p>
