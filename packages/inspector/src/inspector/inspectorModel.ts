...
const FIELDS: FieldSpec[] = [
	...
	// Zach: "In figma panel we should have control of opacity of the stroke,
	// right now we don't." Figma gives every paint its own opacity; tldraw gives
	// the shape one, so this is the outline's half and it lives in meta like the
	// fill's. Gated on HAS_OUTLINE for the same reason the colour is.
	paintField('strokeOpacity', 'Stroke alpha', 'stroke', 'strokeOpacity', 'number', HAS_OUTLINE, {
		min: 0, max: 1, step: 0.01,
		caption: 'Exact stroke', paired: true, glyph: 'opacity', fallback: 1,
		hint: 'Transparency for the outline alone — the interior and label stay opaque.',
	}),
