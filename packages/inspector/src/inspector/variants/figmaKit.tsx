 ...
 */
/** Fold a row's opacity back into the hex the picker shows, so its alpha
 *  handle starts where the row's own number says it is. */
function withAlpha(hex: string, alphaControl?: InspectorControl): string {
	const base = hex.slice(0, 7)
	if (!alphaControl || typeof alphaControl.value !== 'number') return hex
	// Same 0-1 fraction on the way back in.
	const clamped = Math.max(0, Math.min(1, alphaControl.value))
...
export function ColorPickerPopover({
	...
	/**
	 * The row's own opacity field, when it has one.
	 *
	 * WHY the picker needs it: its alpha slider and that field are the SAME
	 * quantity shown twice, and they were not talking to each other — dragging
	 * the slider to 46% left the row still reading 100%, which Zach hit
	 * immediately. The slider wrote an 8-digit hex into the exact-colour
	 * override while the field read a separate `fillOpacity`. Splitting the
	 * alpha out on write, and folding it back in on read, makes one value with
	 * two views instead of two values that disagree.
	 */
	alphaControl?: InspectorControl
	/**
	 * Show ONLY the stock palette — no gradient, no hue strip, no hex field.
	 *
	 * WHY: in stock mode there is no exact colour to pick. A saturation square
	 * that cannot produce a value the record may hold is a control that lies,
	 * and Zach asked for it gone: "On the stock color picker can you hide the
	 * color picker part and just display the stock colors?" The fill-STYLE tiles
	 * stay — `fill` is a stock prop, not an override.
	 */
	stockOnly?: boolean
}) {
		...
		<Popover>
			...
			<PopoverContent className="flex w-56 flex-col gap-2 p-2">
				...
				<HexAlphaColorPickerLazy
					color={withAlpha(hex === '#00000000' ? '#000000ff' : hex, alphaControl)}
					onChange={(next) => {
						if (!exactControl) return
						// Split: the RGB half is the colour, the alpha half is the row's
						// opacity. Written separately so the number beside the swatch and
						// the slider in the picker can never disagree.
						const rgb = next.slice(0, 7)
						ctx.onChange(exactControl.id, rgb)
						if (alphaControl && next.length >= 9) {
							// `fillOpacity` is a 0-1 FRACTION (inspectorModel.ts: min 0, max
							// 1, step 0.01) that the field renders as a percent. Writing 40
							// here put 4,000 in the row — the control multiplies for display,
							// so the value handed to it must not be pre-multiplied.
							const alpha = parseInt(next.slice(7, 9), 16)
							ctx.onChange(alphaControl.id, Math.round((alpha / 255) * 100) / 100)
						}
					}}
				/>
				<div className="flex items-center gap-1.5">
					...
					{exactControl ? (
						<div className="w-14 shrink-0">
							<ScrubNumber
								// WHY this prefers `alphaControl`: the row's opacity field, this
								// number and the picker's alpha slider are ONE quantity shown
								// three times. It used to read the alpha channel of the exact
								// hex, which the slider no longer writes (the alpha is split
								// out on write so the row can show it), so it sat at 100 while
								// the other two moved. Falls back to the hex channel for shape
								// types that have no separate opacity control.
								value={alphaControl && typeof alphaControl.value === 'number'
									? Math.round(alphaControl.value * 100)
									: typeof exactControl.value === 'string' ? hexAlphaPercent(exactControl.value) : null}
								...
								onChange={(percent, gestureStart) => {
									if (alphaControl) { ctx.onChange(alphaControl.id, percent / 100, gestureStart); return }
									ctx.onChange(exactControl.id, withHexAlphaPercent(String(exactControl.value || swatch || '#000000'), percent), gestureStart)
								}}
							/>
						</div>
					) : null}
				...
				{namedControl ? (
					<div className="flex flex-col gap-1">
						<span className={fieldGroupLabelClass}>Stock colors</span>
