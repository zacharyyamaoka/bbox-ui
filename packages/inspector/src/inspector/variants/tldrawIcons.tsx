...
function recolored(svg: string): string {
	...
	// WHY a viewBox has to be ADDED before the width/height are stripped:
	// tldraw ships these as 30x30 SVGs with NO viewBox. Setting
	// `width:100%;height:100%` on a viewBox-less SVG resizes the viewport but
	// not the artwork, so in a 16px box the icon is CROPPED to its top-left
	// corner rather than scaled — which is exactly what Zach saw, six fill
	// styles rendered as unrecognisable fragments. The intrinsic size is read
	// off the original attributes rather than assumed, so an icon that is not
	// 30x30 still scales correctly.
	const width = /(?:<svg\b[^>]*?)\swidth="(\d+)"/.exec(next)?.[1]
	const height = /(?:<svg\b[^>]*?)\sheight="(\d+)"/.exec(next)?.[1]
	if (width && height && !/viewBox=/.test(next)) {
		next = next.replace('<svg ', `<svg viewBox="0 0 ${width} ${height}" `)
	}
