...
export interface PrimitiveOverride {
	...
	fillOpacity?: number
	/** Alpha for the OUTLINE alone, 0-1. Figma gives every paint its own
	 *  opacity; tldraw gives the shape one. This is the stroke's half. */
...
function fillPaint(
	...
	return composite(base, override.fillOpacity)
}

/**
 * The same composite for the OUTLINE.
 *
 * WHY it needs no `resolved` fallback the way the fill does: a stroke alpha
 * with no stroke colour has nothing to tint — the engine's own outline colour
 * is not visible from here, and `color-mix` against an unknown value would be
 * the lie the fill's own WHY already names. So an opacity set without a colour
 * simply does not paint, and the panel shows the field as having no effect
 * rather than inventing one.
 */
function strokePaint(override: PrimitiveOverride): string | undefined {
	if (override.strokeColor === undefined) return undefined
	if (override.strokeOpacity === undefined) return override.strokeColor
	return composite(override.strokeColor, override.strokeOpacity)
}

function composite(base: string, alpha: number): string {
...
export function geoOverrideDisplayValues(
	...
	return defined({
		strokeColor: strokePaint(override),
		strokeWidth: width,
