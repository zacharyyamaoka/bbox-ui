 ...
 * moved viewport.
 */
export function Inspector({ extraControls, ...props }: TLUiStylePanelProps & {
	/**
	 * WHY this seam exists: anything a host app used to mount in
	 * `components.SharePanel` (bbox-ui's Compare button, the lab's own
	 * StockCheckButton before judge round 2) is a real flow SIBLING stacked
	 * above `DefaultStylePanel` in tldraw's top-right column, and it pushes
	 * the style panel down (measured y:8 -> y:34 in the lab). Extra chrome
	 * joins the `position: fixed` control cluster below instead — never a
	 * flow sibling of anything tldraw lays out.
	 */
