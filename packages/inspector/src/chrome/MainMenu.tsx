		...
		<DefaultMainMenu>
			<TldrawUiMenuGroup id="lab-file-v3">
				...
				<TldrawUiMenuItem
					id="open-settings"
					label="Settings…"
					// WHY "Comma" spelled out rather than the "," glyph: tldraw's kbd
					// parser splits the string on "," to support alternative shortcuts
					// (kbd-utils.mjs: `str.split(",")[0]`), so "cmd+," becomes "cmd+"
					// and the comma is silently dropped — which is why the menu read
					// "Settings… Ctrl". Any literal comma is unreachable, and the
					// `[[...]]` escape is applied AFTER that split, so it cannot help.
					// Naming the key is the honest option left; inventing a lookalike
					// glyph would be a rendering lie.
					kbd="cmd+[[Comma]]"
