...
export function useGestures(editor: Editor | null): void {
	...
	useEffect(() => {
		...
		// WHY a middle-button watch: on X11/Wayland, pressing the scroll wheel
		// pastes the PRIMARY selection, and the browser delivers that as an
		// ordinary `paste` event — indistinguishable from Ctrl+V at the event
		// level. So this handler was turning every middle-click into "drop the
		// last selected text on the canvas", which is exactly what Zach hit:
		// "When I press the scroll wheel on my mouse it copy pastes." Middle-click
		// belongs to tldraw, which uses it to pan.
		//
		// The timestamp, rather than a boolean, because the paste arrives in a
		// LATER task than the pointerdown; a flag cleared on pointerup would be
		// gone before the paste ever fired.
		let lastMiddleDown = 0
		const onPointerDown = (event: PointerEvent) => {
			if (event.button === 1) lastMiddleDown = Date.now()
		}

		const onPaste = (event: ClipboardEvent) => {
			if (!settings.pasteUnderCursor) return
			if (Date.now() - lastMiddleDown < 1000) return
